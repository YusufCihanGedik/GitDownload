...
# path = f"{PATH}/drive/Dowload/DATA"
# if os.path.exists(path) and os.path.isdir(path):
#     print("hello")
#     shutil.rmtree(path)     

#     os.mkdir("drive/Dowload/DATA")
#     os.mkdir("drive/Dowload/DATA/images")
#     os.mkdir("drive/Dowload/DATA/mask")
...
class LabelingWidget(LabelDialog):
    ...
    def upgrade_source(slef):
        repo_url = "https://github.com/YusufCihanGedik/Aisoftlabeling.git"
        local_dir = r"C:\Users\Gedik\Desktop\Aisoftlabeling\admin_anylabeling2\git"#os.getcwd()
        local_dir1=os.getcwd()
        print(local_dir1)
        if not os.path.exists(local_dir):
            print("Klasör bulunamadı, repo klonlanıyor...")
            repo = Repo.clone_from(repo_url, local_dir)
            print("Repo başarıyla klonlandı.")
        else:
            if not os.listdir(local_dir):
                print("Dizin boş, repo klonlanıyor...")
                repo = Repo.clone_from(repo_url, local_dir)
                print("Repo başarıyla klonlandı.")
            else:
                try:
                    repo = Repo(local_dir)
                    print("Mevcut repo üzerinde çalışılıyor...")
                    origin = repo.remotes.origin
                    origin.pull()  # Repo güncellenir
                    print("Repo güncellendi.")
                except Exception as e:
                    print(f"Git reposu açılırken hata oluştu: {e}")
    ...
    def import_dropped_image_files(self, image_files):
        ...
        for file in image_files:
            ...
            # self.output_dir = f"{PATH}/drive/Dowload/{USER_ID}/mask"
            if self.output_dir:
                label_file_without_path = osp.basename(label_file)
                label_file = osp.join(self.output_dir, label_file_without_path)
    ...
    def scan_all_USER_ID(self,UserIds):
        ...
        for UserId in UserIds:
            images_list.extend(glob.glob(f"drive/Dowload/{UserId}/images/**"))
        return images_list
 
    def import_image_folder(self, dirpath=None, pattern=None, load=True):
        self.actions.open_next_image.setEnabled(True)
        self.actions.open_prev_image.setEnabled(True)

        # if not self.may_continue() or not dirpath:
        #     return
        # dirpath = f"{PATH}/drive/Dowload/{USER_ID}/images"
        self.last_open_dir = dirpath
        # print(self.last_open_dir)
        # self.last_open_dir = "C:/Users/abdus/Desktop/aisoft/etiket_yazilimi/anylabeling/drive/Dowload/Abdullah/images"
        ...
        # self.output_dir = f"{PATH}/drive/Dowload/{USER_ID}/mask"
        # print(self.scan_all_images(dirpath))
        # for filename in self.scan_all_images(dirpath):
        for filename in self.scan_all_USER_ID(UserIds):
            if pattern and pattern not in filename:
                continue
            label_file = osp.splitext(filename)[0] + ".json"
            label_file = label_file.replace("images", "mask")
            # name = label_file.split('\\')[-1]
            # label_file = f"C:/Users/abdus/Desktop/aisoft/etiket_yazilimi/anylabeling/drive/Dowload/Abdullah/mask/{name}"
            # if self.output_dir:
            #     label_file_without_path = osp.basename(label_file)
            #     label_file = osp.join(self.output_dir, label_file_without_path)
